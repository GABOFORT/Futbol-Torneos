...
|---|---|---|---|
| 17/09/2026 | Diseño | **Los escudos ya salen en la tabla de posiciones y en los rankings.** El sistema tiene dos tablas distintas: la del torneo (`_tabla_grupo.html`) ya mostraba el escudo, y la de la liga (`tabla_posiciones.html`) nunca lo tuvo —solo el nombre—, así que en el torneo se veían los logos y en la liga no. Se agregó copiando el patrón que ya usa `porteros.html`: la imagen va **dentro del botón** que abre el perfil, así el logo también es pulsable, y se reusan `escudo_url`, `celda-equipo` y `escudo-mini`, que ya existían. Un equipo sin escudo cargado muestra su **monograma** con iniciales y color, nunca un hueco. También se agregó a **goleadores y asistencias** (`ranking.html`). Ahí hubo que resolver algo: esa vista arma las filas con `.values()` y **no tiene el objeto equipo**, solo su nombre e id. En vez de reescribir la regla del monograma en la vista —que la habría duplicado—, se trae `{id: escudo_url}` en **una sola consulta** con `.only()`, siguiendo el mismo patrón de mapas que la vista ya usa para partidos jugados, equipos por categoría y trofeos. Verificado: 15 consultas en el ranking (no crece por fila), las seis categorías de LIGA PREMIER TABASCO muestran un escudo por equipo, y la tabla del torneo quedó **sin tocar**. | 4 (flujo) |
| 17/09/2026 | Funcionalidad | **Las redes sociales también son de la liga y del torneo.** El 17/09 quedaron solo en el equipo, y al abrir *Crear liga* o *Crear torneo* no aparecían. Como **el torneo guarda su nombre, logo y descripción en la `Liga` que lleva detrás**, bastó con poner los cuatro campos en `Liga` (migración `0026`, solo agrega columnas vacías) para cubrir a los dos. Se muestran en el encabezado junto al nombre: `_banner_liga.html` —que usan **cuatro pantallas**: tabla de posiciones, categorías, equipos y el listado de ligas— y `_cabecera_torneo.html`, así que no hubo que tocar esas pantallas una por una. **Antes de duplicar nada se centralizó lo que ya existía:** el validador, el helper de campos y los dos mixins pasaron a `apps/equipos/redes.py`, que no importa nada del proyecto. **Era obligatorio y no estético**: el mixin vivía en `equipos/forms.py`, que importa `usuarios/forms.py`, y como `LigaForm` vive ahí, dejarlo donde estaba armaba un **import circular**. De paso la propiedad `redes_sociales` dejó de estar escrita en cada modelo y pasó a `ConRedesSociales`, que comparten equipo y liga, y `campo_de_red()` ahora recibe el modelo en vez de tener `Equipo` fijo. La plantilla que pinta los iconos se generalizó a `templates/_redes.html` (recibe las redes y a quién pertenecen) y `_redes_equipo.html` quedó como una línea que la invoca. **La seguridad es la misma, reutilizada**: cada campo acepta solo su dominio y solo `https`, comprobado también en liga y torneo, al crear y al editar. Verificado que los equipos siguen igual y que las 30 pantallas abren sin error, como administrador y como visitante. | 3 (modelo: `red_*` en Liga), 4 (flujo) |
| 17/09/2026 | Funcionalidad + Seguridad | **Redes sociales del equipo.** Cada equipo puede cargar **Instagram, Facebook, X y TikTok**, todas opcionales, y aparecen como iconos debajo del entrenador en el perfil en ventana y en la página completa del equipo; uno sin redes no muestra nada, ni el hueco. Vale para **liga y torneo**, en el alta y en la edición. Cuatro campos nuevos (`red_*`, migración `0012`, solo agrega columnas vacías). La definición vive en **`apps/equipos/redes.py`** —una sola tupla `REDES` con campo, etiqueta, icono, dominios y ejemplo— y de ahí salen el modelo, los formularios y las plantillas. Sigue el patrón de las insignias: **`RedesSocialesMixin`** es el gemelo de `TitulosMixin`, los campos llevan prefijo `red_` para que `_campo.html` los salte, y se pintan juntos en `_redes_campos.html`. **Seguridad:** cada campo acepta **solo su propia red** (`EnlaceDeRed`): exige `https` y mira el **host real** que resuelve el navegador, no el texto, así que `https://instagram.com@sitio-falso.com` se rechaza. Sin esto alguien podía poner una página falsa detrás del icono de Facebook. Se probaron catorce casos —cinco legítimos que entran, nueve ataques que no— y los cuatro ataques principales por los **cuatro caminos** (alta y edición, liga y torneo). **Se encontró y cerró un hueco durante la prueba:** `formfield()` de Django **no copia los validadores del modelo al formulario**. En la edición no importaba —es un `ModelForm` y Django valida la instancia antes de guardar—, pero el **alta de liga** y **todo el torneo** son `Form` normales que crean con `objects.create()`, que no valida nada: cualquier enlace entraba por ahí. `campo_de_red()` ahora toma el validador del propio modelo y se lo pasa al campo del formulario, así hay una sola fuente de verdad. Los enlaces se abren con `rel="noopener noreferrer nofollow"` y los iconos son SVG dibujados en el sitio: no se carga nada de esas redes, así que no rompen la CSP ni les avisan de quién visita la página. En el alta de liga multicategoría **las redes se copian a cada equipo creado**; si después se edita uno, los demás no cambian. | 3 (modelo: `red_*`), 4 (flujo) |
| 17/09/2026 | Formato + Fix | **El reparto en grupos lo hace el sistema, y el impar arma el rol como se hace a mano.** Dos cosas que salieron al usar los grupos con equipos de verdad. **(1) El impar.** `calendario._una_vuelta()` resolvía la cantidad impar agregando un equipo fantasma para volverla par, y descansaba el que le tocara enfrentarlo. Salía un torneo válido, pero **el descanso caía salteado** —con nueve equipos descansaba el tercero, el quinto, el séptimo— y no es así como se arma un rol: la jornada la define **quién descansa**, y descansan en orden. Se separó la rueda en dos (`_rueda_par`, que quedó **idéntica**, y `_rueda_impar`, nueva): en la jornada del que descansa se toman pares a distancias crecientes de él, dando la vuelta al círculo. Con nueve equipos sale `9-2 8-3 7-4 6-5` descansando el 1, después `1-3 9-4 8-5 7-6` descansando el 2. Cada equipo descansa exactamente una vez y queda de local tantas veces como de visitante, sin fantasma. Se comprobó que el par no cambia **ni un partido** en once tamaños a una y dos vueltas. **(2) El reparto.** Con dos o más grupos, *Generar partidos* mandaba a una pantalla aparte a asignarle el grupo a cada equipo. En una liga eso es trabajo inútil: **el reparto sale de la división**, no de un criterio. Nuevo `grupos.repartir()`, que acomoda por bloques y por orden de inscripción, lo más parejo posible, y le da el equipo de más a los primeros grupos —trece en dos grupos son siete y seis; en tres, cinco, cuatro y cuatro—. Lo llama `_generar_por_grupos()` antes de armar el calendario, así que **se genera de un solo clic**. La pantalla de reparto (`categoria_grupos`, su ruta, su plantilla y `RepartoGruposForm`) **se eliminó**. **El torneo relámpago no se tocó**: ahí los grupos se sortean o se acomodan a mano, y sigue pidiendo el grupo al inscribir cada equipo. | 4 (flujo) |
| 14/09/2026 | Fix | **OpenStreetMap bloqueó los mapas: salía "Access blocked · 403" en vez de la cancha.** No fue un cambio propio: lo bloqueó OSM. Su política de uso de tiles **exige la cabecera `Referer`** para poder identificar quién pide los mapas, y el sitio no la manda porque Django trae `SECURE_REFERRER_POLICY = 'same-origin'` por defecto —nunca se configuró, y `same-origin` no está entre los valores que OSM acepta—. Se comprobó contra el servidor real: **sin referente, tres coordenadas distintas devuelven la misma imagen de 6987 bytes** (la del cartel de bloqueo); con referente devuelven mapas distintos de verdad. Se arregló **sin tocar la seguridad del sitio**: la política global sigue en `same-origin`, y solo las peticiones a OSM llevan `strict-origin-when-cross-origin` —que manda el origen (`https://buhosportleague.com`) y nunca la ruta ni los parámetros—. Va por la opción `referrerPolicy` del `L.tileLayer` (Leaflet 1.9.4 la aplica al `<img>` de cada tile) en los tres mapas (`mapa-sedes.js`, `sedes.js`, `aliados.js`) y por el mismo campo en los cuatro `fetch` a **Nominatim**, el buscador de direcciones, que comparte la política. **El `fetch` al propio servidor** —el que guarda la cancha, en `sedes.js`— **se dejó intacto**: es del mismo origen y lleva CSRF. La CSP ya permitía `*.tile.openstreetmap.org` y `nominatim.openstreetmap.org`, así que no hizo falta tocarla. | 2 (arquitectura) |
| 14/09/2026 | Fix | **Los scripts se servían sin versión y el navegador se quedaba con el viejo.** Las hojas de estilo ya se pedían con `{% static_v %}` —que le pega al final la fecha de modificación del archivo, así que al cambiarlo cambia la URL y el navegador lo vuelve a bajar—, pero **los `.js` se cargaban con `{% static %}` a secas** en las nueve pantallas que los usan. Se vio en vivo: tras agregar el selector de grupos, el desplegable seguía apareciendo con *Todos contra todos* marcado porque Chrome conservaba el `categoria_form.js` anterior, que no conocía el campo nuevo. Se pasaron los nueve a `static_v` (`bracket.js`, `siembra.js`, `torneo_form.js`, `categoria_form.js`, `desplegable.js`) en las siete plantillas de torneos y estadísticas más las dos de categorías, y se quitó el `{% load static %}` que quedó sin uso. **Afecta a las dos competencias**, y en particular a `torneo_siembra.html`, que desde el 14/09 la reusa también la liga. `url_estatico()` degrada solo si no encuentra el archivo —devuelve la URL sin versión en vez de fallar— y cachea el resultado fuera de DEBUG, así que no agrega consultas a disco por petición. | 2 (arquitectura) |
| 14/09/2026 | Fix | **Con un solo grupo, la categoría quedaba trabada pidiendo repartir.** Al elegir 1 grupo, los equipos seguían con el campo `grupo` vacío y el sistema avisaba *"Cada grupo necesita al menos 2 equipos. Falta llenar: grupo A (0)"*, mandando a repartir doce equipos a un único destino posible. El diseño de *"el equipo nace sin grupo y se reparte después"* vale cuando hay algo que decidir; con un solo grupo no lo hay. Se resolvió haciendo que **la pertenencia al grupo único sea implícita**: `Categoria.equipos_del_grupo()` devuelve TODOS los equipos cuando `varios_grupos` es falso, y `reparto` y `equipos_sin_grupo` responden en consecuencia. **Implícito y no un valor guardado a propósito**: así no hay un momento del que haya que acordarse —crear la categoría, inscribir un equipo, admitir uno a mitad de temporada— ni estado que pueda quedar desincronizado, y al pasar de tres grupos a uno las letras viejas dejan de estorbar solas. También se cerraron las pantallas que ya no tienen sentido con un grupo: el botón *Repartir en grupos* no aparece, la vista `categoria_grupos` redirige explicando por qué, y `TorneoEquipoForm` deja de pedir el grupo en vez de ofrecer un único radio. Y se corrigieron dos puntos que habrían fallado callados: `grupos.posiciones()` —que la **pantalla de siembra usa siempre**— devolvía el Grupo A vacío, y `clean_grupos` impedía bajar de tres grupos a uno por *"equipos huérfanos"* que con un solo grupo no existen. | 3 (modelo: `equipos_del_grupo`), 4 (flujo) |
| 14/09/2026 | Fix + Formato | **Un solo grupo ya se puede elegir, y la tabla general dejó de salir dos veces.** El selector de grupos ofrecía desde 2 en las ligas y desde 1 en los torneos, con el rango escrito a mano en cada formulario (`range(1, 9)` en `TorneoCategoriaForm`, las constantes del modelo en `CategoriaForm`). Se unificó en **`Categoria.opciones_de_grupos()`**, que ahora es la única fuente para las dos competencias, y `MINIMO_GRUPOS` bajó a **1**. Un grupo no es lo mismo que no tener grupos aunque el calendario salga idéntico: **la liguilla deja de sembrarse sola desde la tabla y pasa a armarla el administrador**, que es justo lo que el formato corrido no permite; por eso se ofrece y por eso la opción lo explica en su propio texto. Al habilitarlo salió a la luz un defecto **que ya existía en los torneos**: con un solo grupo la pantalla mostraba la tabla del *Grupo A* y, debajo, un desplegable *"Ver la tabla general"* con **exactamente la misma tabla repetida**. Se corrigió con la propiedad **`Categoria.varios_grupos`**: el torneo esconde el desplegable y la liga no calcula siquiera las tablas por grupo (`grupos.posiciones()` ya no se llama), así que con un grupo la pantalla queda como la de siempre — que es lo correcto, porque con un grupo la tabla del grupo ES la general. | 3 (modelo: `varios_grupos`, `opciones_de_grupos`), 4 (flujo) |
| 14/09/2026 | Fix | **La tabla de posiciones salía en orden arbitrario.** `tabla.calcular()` ordenaba por `(-pts, -dg, -gf)` y ahí se acababa el criterio. Al arrancar la temporada **todos los equipos empatan en cero**, así que los tres criterios empataban a la vez y el orden quedaba a merced de lo que devolviera Postgres —que no está garantizado y cambia al editar cualquier equipo—. Se comprobó en *LIGA PREMIER TABASCO / Infantil Prima*: la tabla salía `Leon, Tigres Tab, Altas Rn…`, que no es ni alfabético ni orden de alta, y podía cambiar entre dos recargas sin que se jugara nada. Se agregó un **cuarto criterio: el `id` del equipo**, o sea quién se inscribió antes. Se eligió la antigüedad y no el nombre para que la tabla en blanco cuente algo cierto —el orden en que se fueron inscribiendo— y porque `fecha_creacion` solo guarda el día y no distingue entre los que entraron el mismo. Como la regla vive en un solo lugar, quedó parejo en la tabla pública, las tablas por grupo, la general, el perfil del equipo, la ficha de partido y la portada. **Ojo con la liguilla automática**, que siembra desde esta tabla: hasta ahora, dos equipos empatados en todo se ordenaban al azar y entraban al cuadro en cualquier puesto; desde acá el desempate es la antigüedad, que es determinista y repetible. | 4 (flujo) |
| 14/09/2026 | Formato + Reuso | **Las categorías de liga ya se pueden jugar por grupos.** El round robin por grupos existía desde el 19/08 pero solo lo alcanzaban los torneos relámpago, aunque el campo `Categoria.grupos` y todo `apps/partidos/grupos.py` ya eran **de la categoría, no del torneo**. Lo que faltaba era exponerlo y quitarle los dos supuestos de relámpago que traía pegados. En el formulario de categoría, dentro de *Cómo se juega*, entra **`formato`** (campo de pantalla, no del modelo): *Todos contra todos* o *Round robin por grupos*; al elegir el segundo aparece el selector de cuántos grupos (2 a 8, con sus letras A–Z a la vista), con el mismo patrón `data-*` + bloque `.restricciones` que ya usaba la casilla *Categoría libre*. `formato` no se guarda: `clean()` lo traduce a `grupos=0` o `grupos=N`, para que el modelo siga teniendo **una sola fuente de verdad** y no dos campos que puedan contradecirse. Los dos supuestos corregidos en `grupos.generar()`: (1) llamaba a `armar_jornadas()` **sin vueltas**, porque un relámpago es de un día — ahora pasa `categoria.vueltas`, y las categorías de torneo se siguen creando con `VUELTA_UNICA`, así que para ellas no cambia nada; (2) emparejaba **siempre** entre grupos a los que descansan, lo cual en una liga descuadra las tablas, porque ese partido cruzado suma puntos y deja a unos equipos con más jornadas jugadas que otros — ahora lo decide el campo nuevo **`cruces_entre_grupos`** (migración `0025`), que nace en `False` y que `TorneoCategoriaForm.guardar()` fija en `True` para conservar el comportamiento del relámpago. Nueva pantalla **`/categorias/<pk>/grupos/`** (`categoria_grupos` + `RepartoGruposForm`) para acomodar los equipos: **el equipo nace sin grupo** porque el alta de liga es multicategoría —una misma alta lo inscribe en varias— y el grupo es uno por categoría, así que no hay un valor que sirva para todas; se cierra en cuanto hay partidos generados. La liguilla se bifurca: sin grupos sigue la **siembra automática** por tabla (`liguilla.iniciar`), con grupos va a la **manual** (`categoria_sembrar`), que **reutiliza el `SiembraForm`, el `grupos.sembrar()`, el `siembra.js` y la plantilla del torneo** —lo único que se generalizó fue el kicker del encabezado, de `torneo.nombre` a una variable `kicker`—, porque la regla de quién pasa cambia de una liga a otra y ninguna se deja escribir en el código sin quedarse corta. La tabla pública muestra las N tablas de grupo (reusando `_tabla_grupo.html` bajo un envoltorio `.torneo-ui`) más la general. Validación nueva en `Categoria.clean()`: con grupos no puede haber **mini-liguilla** —sale de los puestos 9 a 12 de *una* tabla— y el cupo debe alcanzar para 2 equipos por grupo. | 3 (modelo: `cruces_entre_grupos`), 4 (flujo), 5 (permisos: `categoria_grupos` y `categoria_sembrar` son `@admin_liga_required` y filtran por `ligas_administradas`) |
| 14/09/2026 | Fix | **El calendario armaba las jornadas al revés.** `calendario._una_vuelta()` aplica el método del círculo, que arranca emparejando al primero con el último (1-12) y termina emparejándolo con el segundo (1-2). El orden natural de una liga es el contrario: la jornada de apertura es 1-2, 12-3, 11-4… Se comprobó contra una tabla hecha a mano para 12 equipos y coincidían **las mismas jornadas exactamente, pero numeradas al revés** (la J1 de la hoja era la J11 del sistema, la J2 la J10, y así). Se corrigió invirtiendo la lista al final de `_una_vuelta`. **No cambia ningún enfrentamiento ni la localía** —que sigue alternando jornada a jornada para que nadie sea local toda la temporada—, solo en qué fecha cae cada partido, y **no toca los calendarios ya generados**, que están guardados en la base. | 4 (flujo) |
